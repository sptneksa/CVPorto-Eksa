...
window.DB_CACHE = { siteData: {}, portfolioData: [], docsData: [] };

window.SiteData = {
  get(){ return window.DB_CACHE.siteData || {}; },
  save(d){ 
    window.DB_CACHE.siteData = d; 
    this._pushToServer();
  },
  section(key){ const d=this.get(); return JSON.parse(JSON.stringify(d[key]!==undefined?d[key]:DEFAULT_DATA[key])); },
  saveSection(key,val){ const d=this.get(); d[key]=val; this.save(d); },
  getPortfolio(){ return (window.DB_CACHE.portfolioData || []).map(migratePortfolioItem); },
  savePortfolio(items){ window.DB_CACHE.portfolioData = items; this._pushToServer(); },
  getDocs(){ return window.DB_CACHE.docsData || []; },
  saveDocs(items){ window.DB_CACHE.docsData = items; this._pushToServer(); },
  async _pushToServer(){
    try {
      const res = await fetch('/api/data', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(window.DB_CACHE)
      });
      if (!res.ok) throw new Error('Gagal menyimpan ke server');
    } catch(e) {
      if(window.showToast) window.showToast('❌ Gagal menyimpan ke server: ' + e.message, 'error');
    }
  }
};
...
const DCats={klinis:'🏥 Klinis',profesional:'💼 Profesional',pelatihan:'📚 Pelatihan',organisasi:'👥 Organisasi',lainnya:'📁 Lainnya'};
let pFilter='all', dFilter='all', _firstRender=true;
...
window.Render = {
  ...
  docs(){
    const data=SiteData.getDocs(), c=document.getElementById('doc-grid'), emp=document.getElementById('doc-empty');
    if(!c)return;
    const filtered=dFilter==='all'?data:data.filter(x=>x.category===dFilter);
    c.querySelectorAll('.doc-item').forEach(el=>el.remove());
    if(!filtered.length){ if(emp)emp.style.display='block'; return; }
    if(emp)emp.style.display='none';
    filtered.forEach(x=>{
      const gi=data.indexOf(x), isVid=x.type&&x.type.startsWith('video/'), el=document.createElement('div');
      el.className='doc-item';
      el.innerHTML=`<div class="doc-item-media">${isVid?`<video src="${x.data}" muted loop playsinline></video><div class="play-icon">▶</div>`:`<img src="${x.data}" alt="${x.title}" loading="lazy">`}</div>
        <div class="doc-item-info"><div class="doc-item-category">${DCats[x.category]||x.category}</div><div class="doc-item-title">${x.title}</div>
        ${x.desc?`<div class="doc-item-desc">${x.desc}</div>`:''}${x.dateStr?`<div class="doc-item-date">📅 ${x.dateStr}</div>`:''}</div>
        <div class="item-admin-controls" onclick="event.stopPropagation()"><button class="item-edit-btn" onclick="Editors.editDoc(${gi})">✏️</button><button class="item-delete-btn" onclick="Editors.deleteDoc(${gi})">🗑️</button></div>`;
      el.addEventListener('click',()=>Lightbox.open(x));
      if(isVid){const v=el.querySelector('video');el.onmouseenter=()=>v.play();el.onmouseleave=()=>{v.pause();v.currentTime=0;}}
      c.appendChild(el);
    });
  },
  ...
  all(){ this.hero();this.skills();this.education();this.experience('clinical');this.experience('professional');this.projects();this.portfolio();this.docs();this.certs();this.awards();this.contact(); }
};
...
window.Lightbox = {
  open(item){
    ...
    if(lcat) lcat.textContent=PCats[item.category]||DCats[item.category]||item.category||'';
...
function initFilters(){
  ...
  document.querySelectorAll('#doc-filters .filter-btn').forEach(b=>b.addEventListener('click',()=>{
    document.querySelectorAll('#doc-filters .filter-btn').forEach(x=>x.classList.remove('active'));b.classList.add('active');dFilter=b.dataset.filter;Render.docs();
  }));
}
...
document.addEventListener('DOMContentLoaded', async ()=>{
  try {
    const res = await fetch('/api/data');
    if(res.ok) window.DB_CACHE = await res.json();
  } catch(e) { console.error('Failed to load data', e); }

  Render.all();
  initParticles();initNavbar();initTabs();initBackToTop();initCounters();initTyping();initHeroPhoto();initFilters();checkAdminHash();
  setTimeout(()=>{initScrollReveal();_firstRender=false;},100);
  if(typeof Admin!=='undefined'&&Admin.checkSession)Admin.checkSession();
